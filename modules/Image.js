...
export async function output_image(imagePath, predictions) {
    ...
    fs.writeFileSync(`data/outputs/${imageFileName.slice(0, -4)}.png`, buffer)
}
...
export function extract_objects(imagePath, predictions) {
    ...
    for (var i = 0; i < predictions.length; i++) {
        ...
        let pred = predictions[i],
            ...
            x_max = pred["x_max"],
            confidence = pred["confidence"],
            outputImage = `data/outputs/${imageFileName.slice(0, -4)}_${i + 1}_${label}_${confidence.toFixed(2)}.png`
        ...
        sharp(imagePath).extract({
            ...
            .then(new_file_info => {
                console.log(`Image saved with a detection of a ${label} with ${confidence.toFixed(2)*100}% confidence.`);
            })
            .catch(err => {
                console.log(err);
